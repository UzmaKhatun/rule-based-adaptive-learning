...
GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxf7sgW__ku-J0dS4Qiw9b55LQt2U5N67s09vOI_1My_iS_o49FWEgeYQy6SJQZemi5Hg/exec"

def log_to_google_sheets(event_type, data):
    ...
    with col2:
        ...
        if st.button("🚀 Start Adventure!", use_container_width=True, type="primary"):
            if player_name.strip():
                ...
                log_to_google_sheets('SESSION_START', {
...
def summary_screen():
    ...
    if 'completion_logged' not in st.session_state:
        log_to_google_sheets('SESSION_COMPLETE', {
            'player_name': st.session_state.player_name,
            'accuracy': f"{stats['accuracy']}%",
            'avg_time': f"{stats['average_time']}s",
            'difficulty': st.session_state.engine.current_difficulty
        })
